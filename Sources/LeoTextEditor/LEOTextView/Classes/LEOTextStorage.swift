...
class LEOTextStorage: NSTextStorage {
    ...
    override func processEditing() {
        if isChangeCharacters && editedRange.length > 0 {
            ...
            performReplacementColorForRange(editedRange, mode: textView.inputColorMode)
        }
    ...
    func performReplacementColorForRange(_ range: NSRange, mode: LEOInputColorMode) {
        if range.length > 0 {
            var attrValue: UIColor!
            if mode == LEOInputColorMode.highlight {
                attrValue = textView.highlightColor
                safeAddAttributes([NSAttributedString.Key.backgroundColor : attrValue], range: range)
                return
            }
            if mode == LEOInputColorMode.stroke {
                attrValue = textView.strokeColor
                safeAddAttributes([NSAttributedString.Key.foregroundColor : attrValue], range: range)
                return
            }
        }
    }

    func performReplacementStyleForRange(_ range: NSRange, mode: LEOInputStyleMode) {
        if range.length > 0 {
            var attrValue = NSUnderlineStyle.thick.rawValue as AnyObject
            if mode == LEOInputStyleMode.strike {
                safeAddAttributes([NSAttributedString.Key.strikethroughStyle : attrValue], range: range)
                return
            }
            if mode == LEOInputStyleMode.underline {
                safeAddAttributes([NSAttributedString.Key.underlineStyle : attrValue], range: range)
                return
            }
            if mode == LEOInputStyleMode.none {
                attrValue = NSUnderlineStyle(rawValue: 0).rawValue as AnyObject
                safeAddAttributes([NSAttributedString.Key.strikethroughStyle : attrValue], range: range)
                safeAddAttributes([NSAttributedString.Key.underlineStyle : attrValue], range: range)
                return
            }
    ...
    func performReplacementClearForRange(_ range: NSRange) {
        if range.length > 0 {
            safeAddAttributes([NSAttributedString.Key.font : textView.normalFont], range: range)
            safeAddAttributes([NSAttributedString.Key.backgroundColor : UIColor.clear], range: range)
            safeAddAttributes([NSAttributedString.Key.foregroundColor : UIColor.label], range: range)
            safeAddAttributes([NSAttributedString.Key.strikethroughStyle : NSUnderlineStyle(rawValue: 0).rawValue as AnyObject], range: range)
            safeAddAttributes([NSAttributedString.Key.underlineStyle : NSUnderlineStyle(rawValue: 0).rawValue as AnyObject], range: range)
        }
    }

    // MARK: - Undo & Redo support
    
    func undoSupportChangeWithRange(_ range: NSRange, toMode targetMode: Int, currentMode: Int, isColor: Bool = false) {
        textView.undoManager?.registerUndo(withTarget: self, handler: { (type) in
            self.undoSupportChangeWithRange(range, toMode: targetMode, currentMode: currentMode, isColor: isColor)
        })
        if textView.undoManager!.isUndoing {
            if isColor {
                performReplacementColorForRange(range, mode: LEOInputColorMode(rawValue: currentMode)!)
            } else {
                performReplacementsForRange(range, mode: LEOInputFontMode(rawValue: currentMode)!)
            }
        } else {
            if isColor {
                performReplacementColorForRange(range, mode: LEOInputColorMode(rawValue: targetMode)!)
            } else {
    ...
    func undoSupportStyleChangeWithRange(_ range: NSRange, toMode targetMode: Int, currentMode: Int) {
        textView.undoManager?.registerUndo(withTarget: self, handler: { (type) in
            self.undoSupportStyleChangeWithRange(range, toMode: targetMode, currentMode: currentMode)
        })
        if textView.undoManager!.isUndoing {
            performReplacementStyleForRange(range, mode: LEOInputStyleMode(rawValue: currentMode)!)
        } else {
            performReplacementStyleForRange(range, mode: LEOInputStyleMode(rawValue: targetMode)!)
        }
    }
